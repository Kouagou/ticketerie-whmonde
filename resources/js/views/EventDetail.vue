<template>
    <div class="container mx-auto px-4 py-8">
        ...
        <div v-else-if="!event" class="text-center py-12">
            <p class="text-xl text-red-600">Événement non trouvé</p>
                ...
                <div>
                    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        <div class="p-6">
                            ...
                            <div v-if="registrationSuccess" class="mt-4 p-3 bg-green-100 text-green-800 rounded-md">
                                <p class="font-medium">Inscription réussie !</p>
                                <p>Un email contenant votre ticket a été envoyé à l'adresse indiquée.</p>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    setup() {
        ...
        const isEventFull = computed(() => {
            return event.value && event.value.current_participants >= event.value.max_participants;
        });

        const isEventActive = computed(() => {
            return event.value && event.value.status === 'active';
        });

        onMounted(async () => {
            try {
                const eventData = await eventStore.fetchEvent(route.params.id);
                if (eventData) {
        ...
        const registerForEvent = async () => {
            if (isEventFull.value || !isEventActive.value) {
                return;
            }
